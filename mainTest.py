...
if __name__ == '__main__':
    ...
    for k in range(4):
        if(k != 8):
            ...
            print(np.shape(_occlusion))
            _occlusion_Up = np.zeros((4,int(width*4), int(height*4)))
            for i in range(4):    
                _occlusion_Up[i] = cv2.resize(_occlusion[i], (int(width*4), int(height*4)), interpolation=cv2.INTER_LINEAR)
            both_occlusion_Up = cv2.resize(both_occlusion, (int(width*4), int(height*4)), interpolation=cv2.INTER_LINEAR)
            warpUp = np.zeros((int(width*4), int(height*4), 3))
            new_sub = sub_re.astype(np.int32)
            # both_occlusion[i, j] != 1   _occlusion[tmp, i, j] != 1
            if(diffOcc):
                if(k == 0 or k == 4 or k == 12 or k == 16):
                    tmp = 0
                elif(k == 1 or k == 5 or k == 11 or k == 15):
            ...
            if(k >= 0 and k <= 3):
                for i in range(height*4):
                    for j in range(width*4):
                        if((i + dispUp[i, j]) < height*4 and both_occlusion_Up[i, j] != 1):
                            warpUp[i + dispUp[i, j], j, :] = difUp[i, j, :]

            elif(k >= 4 and k <= 7):
                for i in range(height*4):
                    for j in range(width*4):
                        if((j + dispUp[i, j]) < width*4 and both_occlusion_Up[i, j] != 1):
                            warpUp[i, j + dispUp[i, j], :] = difUp[i, j, :]

            elif(k >= 9 and k <= 12):
                for i in range(height*4):
                    for j in range(width*4):
                        if((j - dispUp[i, j]) >= 0 and both_occlusion_Up[i, j] != 1):
                            warpUp[i, j - dispUp[i, j], :] = difUp[i, j, :]

            elif(k >= 13 and k <= 16):
                for i in range(height*4):
                    for j in range(width*4):
                        if((i - dispUp[i, j]) >= 0 and both_occlusion_Up[i, j] != 1):
                            warpUp[i - dispUp[i, j], j, :] = difUp[i, j, :]
